...
//Marcelo Celi - 20/03/2022 - ESPECIFICO BOMBAY
Private cCodEan  := ""
Private cCodtrib := ""
Private lativaCom := .F.
			...
			Else
				...
				If !SF2->F2_TIPO $ "DB" 
					...
					*/
					...
					// Marcelo Celi - 04/04/2022 - ESPECIFICOS BOMBAY
					If !Empty(SA1->A1_XMSGFNF) 	
						cMensCli :=	" MsgCli: "  +alltrim(SA1->A1_XMSGFNF)		
					EndIf
					...
					//Verifica se existe Template DCL
	      			IF cVerAmb >= "4.00" .And. (ExistTemplate("PROCMSG")) //Tratativa para Grupo de Repasse de Combustiveis
						BeginSql Alias cAliasSD2
								// Marcelo Celi - 21/03/2022 - ESPECIFICOS BOMBAY
								/*
								...
								*/
								...
								D2_QUANT,D2_TOTAL,D2_DESCON,D2_VALFRE,D2_SEGURO,D2_PEDIDO,D2_ITEMPV,D2_DESPESA,D2_VALBRUT,D2_VALISS,D2_PRUNIT,
								D2_CLASFIS,D2_PRCVEN,D2_IDENTB6,D2_CODISS,D2_DESCZFR,D2_PREEMB,D2_DESCZFC,D2_DESCZFP,D2_LOTECTL,D2_NUMLOTE,D2_ICMSRET,D2_VALPS3,
								D2_ORIGLAN,D2_VALCF3,D2_VALIPI,D2_VALACRS,D2_PICM,D2_PDV,D2_BRICMSO,D2_ICMRETO,D2_BRICMSD,D2_ICMRETD,D2_CSOSN,D2_QTSEGUM,D2_SEGUM %Exp:cField% 
								FROM %Table:SD2% SD2
								WHERE
								SD2.D2_FILIAL  = %xFilial:SD2% AND
								SD2.D2_SERIE   = %Exp:SF2->F2_SERIE% AND
					...
					Else
						BeginSql Alias cAliasSD2
							...
							*/
							...
							D2_QUANT,D2_TOTAL,D2_DESCON,D2_VALFRE,D2_SEGURO,D2_PEDIDO,D2_ITEMPV,D2_DESPESA,D2_VALBRUT,D2_VALISS,D2_PRUNIT,
							D2_CLASFIS,D2_PRCVEN,D2_IDENTB6,D2_CODISS,D2_DESCZFR,D2_PREEMB,D2_DESCZFC,D2_DESCZFP,D2_LOTECTL,D2_NUMLOTE,D2_ICMSRET,D2_VALPS3,
							D2_ORIGLAN,D2_VALCF3,D2_VALIPI,D2_VALACRS,D2_PICM,D2_PDV,D2_CSOSN,D2_QTSEGUM,D2_SEGUM %Exp:cField% 
							FROM %Table:SD2% SD2
							WHERE
							SD2.D2_FILIAL  = %xFilial:SD2% AND
							SD2.D2_SERIE   = %Exp:SF2->F2_SERIE% AND
				...
				While !Eof() .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
					...
					If lContinua
						...
						*/
						...
						//->> Marcelo Celi - 20/03/2022 - ESPECIFICO BOMBAY
						cUMCUS := POSICIONE("SC6",1,xFilial("SD2") + (cAliasSD2)->D2_PEDIDO + (cAliasSD2)->D2_ITEMPV + (cAliasSD2)->D2_COD,"C6_XUMVEN")
						lativaCom := IIF(cUMCUS==(cAliasSD2)->D2_SEGUM,.T.,.F.)  //Marcelo Celi - 20/03/2022 -  Unidade Comercial - ESPECIFICO BOMBAY
						...
						aadd(aProd,	{Len(aProd)+1,;
							cCodProd,;	
							iif(!lativaCom,cValToChar(SB1->B1_CODGTIN),cValToChar(SB1->B1_XCODDUN))/*IIf(Val(SB1->B1_CODBAR)==0,"",StrZero(Val(SB1->B1_CODBAR),Len(Alltrim(SB1->B1_CODBAR)),0))*/,;
							cDescProd + IIF(lativaCom,cValToChar(SB1->B1_XCODDUN),cValToChar(SB1->B1_CODGTIN)),;
							SB1->B1_POSIPI,;//Retirada validação do parametro MV_CAPPROD, de acordo com a NT2014/004 não é mais possível informar o capítulo do NCM
							SB1->B1_EX_NCM,;
							cD2Cfop,;
							IIF(lativaCom,(cAliasSD2)->D2_SEGUM,SB1->B1_UM),;
							IIF(lativaCom,(cAliasSD2)->D2_QTSEGUM,(cAliasSD2)->D2_QUANT),;
							...
							MyretUn2UM( lNoImp2UM, cCFOPExp, Alltrim((cAliasSD2)->D2_CF),SB5->B5_UMDIPI,IIF(lativaCom,(cAliasSD2)->D2_SEGUM,SB1->B1_UM)),;
						    MyretQtd2UM( lNoImp2UM, cCFOPExp, Alltrim((cAliasSD2)->D2_CF),SB5->B5_CONVDIP,IIF(lativaCom,(cAliasSD2)->D2_QTSEGUM,(cAliasSD2)->D2_QUANT)),;
							(cAliasSD2)->D2_VALFRE,;
							(cAliasSD2)->D2_SEGURO,;
							...
							IIf(SB1->(ColumnPos("B1_CODBAR")) > 0,IIf(Val(SB1->B1_CODBAR)==0,"",StrZero(Val(SB1->B1_CODBAR),Len(Alltrim(SB1->B1_CODBAR)),0)),""),;//aprod[46]  Código de barra da segunda unidade de medida.  // Marcelo Celi - 28/04/2022 - Especificos Bombay - IIf(SB1->(ColumnPos("B1_CODGTIN")) > 0,SB1->B1_CODGTIN,""),;  //aprod[46]
...
//->> Marcelo Celi - 20/03/2022 - ESPECIFICO BOMBAY - Flavio solicitou alterar array de itens de notas de saida, aglutinando os itens...
//If cTipo == "1"
//	aProd := AglutProds(aProd)
//EndIf
...
/*/{protheus.doc} AglutProds
*******************************************************************************************
ESPECIFICO BOMBAY

Função especifica para tratar e aglutinar produtos no array da nota de saida
...
*******************************************************************************************
/*/
Static Function AglutProds(aProd)
Local aNewProd  := {}
Local lAglutina := Upper(Alltrim(GetNewPar("MC_AGLUVDA","S"))) == "S"
Local nX		:= 0
Local nPos		:= 0
Local nItem		:= 1
...
If lAglutina
	For nX:=1 to Len(aProd)
		nPos := Ascan(aNewProd,{|x| Alltrim(x[02])==Alltrim(aProd[nX,02]) .And. Alltrim(x[27])==Alltrim(aProd[nX,27]) }) // Produto e Tes
		If nPos == 0
			aAdd(aNewProd,aProd[nX])
			aNewProd[Len(aNewProd)][01] := nItem	// redefinir o item
			aNewProd[Len(aNewProd)][19] := "" 		// limpar o lote
			aNewProd[Len(aNewProd)][20] := "" 		// limpar o endereço
			nItem++
		Else
			aNewProd[nPos][09] += aProd[nX][09] // quantidade
			aNewProd[nPos][10] += aProd[nX][10] // valor total			
			aNewProd[nPos][12] += aProd[nX][12] 
			aNewProd[nPos][13] += aProd[nX][13] 
			aNewProd[nPos][14] += aProd[nX][14] 
			aNewProd[nPos][15] += aProd[nX][15] 
			aNewProd[nPos][16] += aProd[nX][16] 

			aNewProd[nPos][22] += aProd[nX][22] 
			aNewProd[nPos][26] += aProd[nX][26] 
			aNewProd[nPos][30] += aProd[nX][30] 
			aNewProd[nPos][31] += aProd[nX][31] 
			aNewProd[nPos][32] += aProd[nX][32] 
			aNewProd[nPos][35] += aProd[nX][35] 
			aNewProd[nPos][36] += aProd[nX][36] 
			aNewProd[nPos][37] += aProd[nX][37] 
			aNewProd[nPos][43] += aProd[nX][43] 
		EndIf
	Next nX
Else
	aNewProd := aProd
...
*******************************************************************************************
/*/
Static Function MyretQtd2UM( lNoImp2UM, cCFOPExp, cCFOP, nCONVDIP, nQUANT)
...
*******************************************************************************************
/*/
Static Function MyretUn2UM( lNoImp2UM, cCFOPExp, cCFOP, cUMDIPI, cUM)
