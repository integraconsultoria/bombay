...
User Function IntRFFluxc()
	...
	aRetParam := {cFilAnt,cFilAnt,30,dDatabase,Space(150),Replicate(" ",Tamsx3("E5_BANCO")[01]),Replicate(" ",Tamsx3("E5_AGENCIA")[01]),Replicate(" ",Tamsx3("E5_CONTA")[01]),Replicate("Z",Tamsx3("E5_BANCO")[01]),Replicate("Z",Tamsx3("E5_AGENCIA")[01]),Replicate("Z",Tamsx3("E5_CONTA")[01]),2,3,.F.,.T.,.T.,.T.,.F.,.T.,Space(150),Space(150),Space(150),Space(150)}
	...
	aAdd(aParambox,{2,"Considerar Fluxo" 								,aRetParam[12],aTpData,100,".T.",.T.})
    ...
    aAdd(aParambox,{1,"Desconsiderar Clientes + Loja (Sep por ;)" 		,aRetParam[20],"@!","","" 		,".T.",180 ,.F.	})
    aAdd(aParambox,{1,"Desconsiderar Fornecedores + Loja (Sep por ;)"	,aRetParam[21],"@!","","" 		,".T.",180 ,.F.	})
    aAdd(aParambox,{1,"Desconsiderar Prefixos a Receber (Sep por ;)"	,aRetParam[22],"@!","","" 		,".T.",180 ,.F.	})
    aAdd(aParambox,{1,"Desconsiderar Prefixos a Pagar (Sep por ;)"	    ,aRetParam[23],"@!","","" 		,".T.",180 ,.F.	})
...
Static Function GetaCols(nTipo,dDataDe,dDataAte,lJob,aReceb,aPgto)
	...
	If nTipo == 0
		...
		For nX:=1 to Len(aReceb)
			//->> Extrai Detalhes
			IncProc("Extraindo Movimentos a Receber...")
			nPosData := Ascan(aDadData,{|x| x==aReceb[nX,08]})
			If nPosData>0
				If Valtype(aReceb[nX,09])=="N" .And. Valtype(aDadTmp[nPosData])=="N"
					aDadTmp[nPosData] += aReceb[nX,09]

					//->> Distrinui nos demais dias ate ao fim
					For nY:=nPosData+1 to Len(aDadTmp)-2
						If Valtype(aDadTmp[nY])=="N"
							aDadTmp[nY] += aReceb[nX,09]
						Else
							nX:=nX
						EndIf	
					Next nY
		...
		For nX:=1 to Len(aPgto)
			//->> Extrai Detalhes
			IncProc("Extraindo Movimentos a Pagar...")
			nPosData := Ascan(aDadData,{|x| x==aPgto[nX,08]})
			If nPosData>0
				If Valtype(aPgto[nX,09])=="N" .And. Valtype(aDadTmp[nPosData])=="N"
					aDadTmp[nPosData] -= aPgto[nX,09]

					//->> Distrinui nos demais dias ate ao fim
					For nY:=nPosData+1 to Len(aDadTmp)-2
						If Valtype(aDadTmp[nY])=="N"
							aDadTmp[nY] += aPgto[nX,09]
						Else
							nX:=nX
						EndIf	
					Next nY
...
Static Function GetaHeader(nTipo,aCabec,aRodape,dDataDe,dDataAte)
	...
	If nTipo == 0
		//->> Cabecalho
		If aRetParam[12] <> 1		
			aAdd(aCabec,{2,Upper("Fluxo Diario Realizados entre "+dToc(dDataDe)+" e "+dToc(dDataAte)),"@E!",200,"C","s01"})
		Else
			aAdd(aCabec,{2,Upper("Fluxo Diario a Realizar entre "+dToc(dDataDe)+" e "+dToc(dDataAte)),"@E!",200,"C","s01"})
		EndIf
	...
	If nTipo == 4
		...
		aAdd(aHeader,{"Codigo"	 ,"CPO1", "@E!",060,"C",'"s02"',"s01","N"})
		aAdd(aHeader,{"Descrição","CPO1", "@E!",140,"C",'"s02"',"s01","N"})
		nCpo := 3
		For dX := dDataDe to dDataAte
			aAdd(aHeader,{"","CPO"+Alltrim(Str(nCpo)), "@E!",080,"N",'"s02"',"s01","N"})
			nCpo++
		...
		If !Empty(cDoc)
			...
			If (cAlias)->(!Eof()) .And. (cAlias)->(!Bof())
				Do While (cAlias)->(!Eof())
					...
					(cAlias)->DESCRICAO,; // 02 - DESCRIï¿½ï¿½O DO GRUPO
			...
			Else
				...
				"MOVIMENTOS SEM CLASSIFICACAO",	; // 02 - DESCRIï¿½ï¿½O DA NATUREZA
				aMovimento[09],					; // 03 - VALOR TOTAL DO TITULO
				100}							) // 04 - PERCENTUAL DE USO
			EndIf
			(cAlias)->(dbCloseArea())
			...
			If (cAlias)->(!Eof()) .And. (cAlias)->(!Bof())
				Do While (cAlias)->(!Eof())
					...
					If CTD->(dbSeek(xFilial("CTD")+Left((cAlias)->ITEM,4)))
						...
						Alltrim(CTD->CTD_DESC01)+If(!Empty(cTitPai)," - TIT PRINC:"+cTitPai,""),	; // 02 - DESCRIï¿½ï¿½O DO ITEM
